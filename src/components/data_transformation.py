...
from src.constants import TARGET_COLUMN, SCHEMA_FILE_PATH, CURRENT_YEAR,CURRENT_DATE
...
class DataTransformation:
    ...
    def create_new_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            encoder = ColumnTransformer(
                transformers=[
                    ('ohe', OneHotEncoder(sparse_output=False, drop=None, handle_unknown='ignore'), valid_categorical_cols)
                ],
                remainder='passthrough',
            ...
            logging.info("Exited create_new_features method of DataTransformation class")
    ...
    def _drop_id_column(self, df):
        ...
        logging.info("Dropping columns if present in schema")
        
        drop_cols = self._schema_config['drop_columns']  # e.g., ["id", "policy_number"]
        
        # Only drop those columns that are actually in the DataFrame
