...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: DataFrame) -> tuple[bool, list]:
        """
        Method Name :   validate_number_of_columns
        Description :   This method validates the number of expected columns in the dataframe.

        Output      :   Tuple (status: bool, missing_columns: list)
        ...
        try:
            ...
            logging.info(f"Validation status: {status}, Missing columns: {missing_columns}")
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            validation_status, missing_train_cols = self.validate_number_of_columns(dataframe=train_df)
            if missing_train_cols:
                validation_error_msg += f"Missing columns in training dataframe: {missing_train_cols}. "
            else:
                logging.info("All required columns present in training dataframe.")

            validation_status, missing_test_cols = self.validate_number_of_columns(dataframe=test_df)
            if missing_test_cols:
                validation_error_msg += f"Missing columns in test dataframe: {missing_test_cols}. "
            else:
                logging.info("All required columns present in test dataframe.")

            # Validate schema column types
            missing_train_schema_cols = self.is_column_exist(df=train_df)
